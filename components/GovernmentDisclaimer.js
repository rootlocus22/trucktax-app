  ...
  if (compact) {
    return (
      <p className={`text-xs text-slate-500 ${className}`}>
        easyucr.com is not affiliated with FMCSA or DOT. Independent filing service.
      </p>
    );
  }
  return (
    <div className={`rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 ${className}`}>
      ...
      <p className="mt-0.5">
        easyucr.com is not affiliated with the FMCSA, DOT, IRS, or any government agency. We are a private third-party provider that helps you complete your UCR and compliance filings.
      </p>
    </div>
  );
}
