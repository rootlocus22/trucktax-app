...
export async function generateInvoicePDF(payment, business, user) {
    try {
        ...
        } else if (payment.type === 'ucr_filing' || payment.filingType === 'ucr') {
            filingLabel = 'UCR Registration Renewal';
            // If the payment specifically noted it was a government fee
            if (payment.ucrGovFee || (payment.description && payment.description.includes('Government'))) {
                feeDescription = 'Federal Government Fee';
                itemAmount = payment.ucrGovFee || payment.amount || 0;
            } else {
        ...
        const isGovFee = payment.ucrGovFee || (payment.type === 'ucr_filing' && payment.description?.includes('Government'));
