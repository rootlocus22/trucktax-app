...
export async function generateInvoicePDF(payment, business, user) {
    try {
        ...
        const filingLabel = payment.filingType === 'amendment' ? '2290 Amendment' :
            payment.filingType === 'refund' ? 'Form 8849 Refund' : 'IRS Form 2290 Filing';
        drawText(`${filingLabel} - Professional Service Fee`, 50, y, 11);
        drawText(`$${(payment.serviceFee || payment.baseAmount || 0).toFixed(2)}`, 480, y, 11, boldFont);
        ...
        const serviceSubtotal = (payment.serviceFee || payment.baseAmount || 0);
        ...
        y -= 12;
        drawText('This receipt covers the professional service fees for Form 2290 filing. Any IRS tax liability is separate.', 40, y, 8);
