...
export async function getVehiclesByUser(userId, businessId = null) {
  const vehiclesRef = collection(db, 'vehicles');
  let q;

  if (businessId) {
    // Filter by both userId and businessId
    q = query(
      vehiclesRef,
      where('userId', '==', userId),
...
export async function getVehiclesByBusiness(businessId) {
  const vehiclesRef = collection(db, 'vehicles');
  const q = query(vehiclesRef, where('businessId', '==', businessId));
  const querySnapshot = await getDocs(q);
  return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
}
...
/** Get payment history for a user (from payments collection, e.g. Stripe UCR payments) */
export async function getPaymentsByUser(userId) {
  ...
  return snapshot.docs.map(d => {
    ...
    return {
      ...
      invoiceUrl: data.stripeSessionId ? `https://dashboard.stripe.com/payments/${data.stripeSessionId}` : null,
 ...
 */
export async function deleteFiling(filingId) {
  const filingRef = doc(db, 'filings', filingId);
...
export async function getAgentQueue() {
  try {
    ...
    try {
      const q = query(
        filingsRef,
        where('status', 'in', ['pending_ach', 'submitted', 'processing', 'action_required']),
    ...
    filings = filings.filter(f =>
      f.filingType === 'ucr' && f.status && ['pending_ach', 'submitted', 'processing', 'action_required'].includes(f.status)
    );

    console.log(`Filtered to ${filings.length} UCR filings`);
...
// MCS-150 Submissions collection helpers
/**
 * Create a new MCS-150 submission document
 * @param {string} userId - User ID
 * @param {Object} submissionData - Submission data including PDF URL
 * @returns {Promise<string>} - Submission document ID
 */
export async function createMcs150Submission(userId, submissionData) {
  const submissionsRef = collection(db, 'mcs150Submissions');
...
/**
 * Get MCS-150 submission by ID
 * @param {string} submissionId - Submission document ID
 * @returns {Promise<Object|null>} - Submission document or null
 */
export async function getMcs150Submission(submissionId) {
  const submissionRef = doc(db, 'mcs150Submissions', submissionId);
...
/**
 * Get all MCS-150 submissions for a user
 * @param {string} userId - User ID
 * @returns {Promise<Array>} - Array of submissions
 */
export async function getMcs150SubmissionsByUser(userId) {
  const submissionsRef = collection(db, 'mcs150Submissions');
  const q = query(
    submissionsRef,
    where('userId', '==', userId),
...
/**
 * Get all MCS-150 submissions (for agents/admin)
 * @returns {Promise<Array>} - Array of all submissions with user details
 */
export async function getAllMcs150Submissions() {
  const submissionsRef = collection(db, 'mcs150Submissions');
  ...
  const submissionsWithDetails = await Promise.all(
    submissions.map(async (submission) => {
      ...
      if (submission.userId) {
        try {
          user = await getUser(submission.userId);
        } catch (err) {
          console.error(`Error loading user for MCS-150 submission ${submission.id}:`, err);
        }
      }
...
/**
 * Log detailed payment information for analytics
 * @param {Object} paymentData - The structured payment and acquisition data
...
export async function logPayment(paymentData) {
  try {
    ...
    const docRef = await addDoc(paymentLogsRef, {
      ...paymentData,
      timestamp: serverTimestamp(),
      createdAt: new Date().toISOString()
    });
    console.log('Payment log created:', docRef.id);
    return docRef.id;
  } catch (error) {
    console.error('Error logging payment:', error);
    // Don't throw here to avoid blocking the user flow if logging fails
    return null;
  }
}
 ...
 */
export async function getPaymentLogsByUser(userId) {
  try {
    const paymentLogsRef = collection(db, 'payment_logs');
    const q = query(
      paymentLogsRef,
    ...
    if (error.code === 'failed-precondition' || error.message?.includes('index')) {
      // Index missing error, fallback to client-side sort
      console.warn('Firestore index missing for payment_logs, falling back to client-side sort');
      const paymentLogsRef = collection(db, 'payment_logs');
      const q = query(paymentLogsRef, where('userId', '==', userId));
      const querySnapshot = await getDocs(q);
