...
export async function saveDraftFiling(userId, draftData) {
  ...
  const draftsRef = collection(db, 'ucrFilings');

  const draftFiling = {
    userId,
    ...draftData,
    status: 'draft',
    filingYear: draftData.filingYear || new Date().getFullYear(),
    updatedAt: serverTimestamp()
  };

  // If new draft (no createdAt), add it
  if (!draftData.draftId && !draftData.id) {
    draftFiling.createdAt = serverTimestamp();
  }

  // If draft already exists (has ID), update it; otherwise create new
  const draftId = draftData.draftId || draftData.id;
  if (draftId) {
    const draftDocRef = doc(db, 'ucrFilings', draftId);
    await setDoc(draftDocRef, draftFiling, { merge: true });
    return draftId;
  } else {
