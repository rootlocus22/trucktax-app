...
// Social (from brand)
const SOCIAL = {
  x: 'https://x.com/QuicktruckTax',
  facebook: 'https://www.facebook.com/profile.php?id=61586215259083',
  instagram: 'https://www.instagram.com/quicktrucktax/',
};
...
function buildFooter(recipientEmail) {
  ...
  const unsubBlock = unsubLink
    ? `<br><a href="${unsubLink}" style="color: ${MUTED}; font-size: 11px;">Unsubscribe from marketing emails</a>`
    : '';
  return `
  <strong>${BRAND}</strong><br>
  <span style="font-size: 11px; color: ${NAVY}; font-weight: 600;">${TAGLINE_POSITIONING}</span><br><br>
  <a href="${SOCIAL.x}" style="color: ${ORANGE}; text-decoration: none; font-size: 11px;">X</a> &middot; <a href="${SOCIAL.facebook}" style="color: ${ORANGE}; text-decoration: none; font-size: 11px;">Facebook</a> &middot; <a href="${SOCIAL.instagram}" style="color: ${ORANGE}; text-decoration: none; font-size: 11px;">Instagram</a><br>
  Vendax Systems LLC · 28 Geary St STE 650, San Francisco, CA 94108<br>
  <a href="mailto:support@vendaxsystemlabs.com" style="color: ${ORANGE}; text-decoration: none;">support@vendaxsystemlabs.com</a> · <a href="tel:+13478018631" style="color: ${ORANGE}; text-decoration: none;">+1 (347) 801-8631</a>
  ${unsubBlock}
  <br><br>
...
/** Cold outreach to motor carriers / trucking companies promoting UCR filing service. */
export function getUCROutreachEmailTemplate({ companyName = '', contactName = '', email = '' }) {
  const greeting = contactName ? `Hi ${contactName},` : (companyName ? `Hi ${companyName} team,` : 'Hi there,');
  const subject = `File your 2026 UCR in 5 minutes — easyucr.com`;
  const content = `
    <p style="font-size: 18px; font-weight: 600; color: ${NAVY}; margin-bottom: 16px;">${greeting}</p>
    <p style="color: ${TEXT};">I'm reaching out because every interstate motor carrier operating in the US is required to file <strong>UCR (Unified Carrier Registration)</strong> by December 31 each year.</p>
    <p style="color: ${TEXT};">We built <strong style="color: ${NAVY};">${BRAND}</strong> to make this easy:</p>
    <ul style="margin: 12px 0; padding-left: 20px; color: ${TEXT};">
      <li>File in <strong>under 5 minutes</strong> — enter your DOT number, we do the rest</li>
      <li><strong>$79 service fee</strong> + official government UCR fee in one payment</li>
      <li>We file with UCR.gov and pay the government fee on your behalf</li>
      <li>Certificate delivered to your email and stored in your dashboard</li>
    </ul>
    <p style="color: ${TEXT};">No paperwork, no confusing government portals, no surprises.</p>
    <div style="background: ${BG_PAGE}; border-left: 4px solid ${ORANGE}; padding: 14px 18px; border-radius: 0 8px 8px 0; margin: 20px 0;">
      <p style="margin: 0; color: ${NAVY}; font-weight: 600;">2026 UCR fees start at $46 (0–2 vehicles)</p>
      <p style="margin: 4px 0 0; color: ${MUTED}; font-size: 14px;">One payment = our $79 service fee + government fee. We handle the government submission.</p>
    </div>
