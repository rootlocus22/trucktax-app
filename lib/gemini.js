...
/**
 * Parse fuel receipt from image/PDF using Gemini Vision
 * @param {Buffer} buffer - Image or PDF buffer
 * @param {string} mimeType - MIME type (image/jpeg, image/png, application/pdf)
 * @returns {Promise<Object>} - Extracted receipt data
 */
export async function parseFuelReceipt(buffer, mimeType = 'image/jpeg') {
  ...
  const prompt = `Extract the following information from this fuel receipt image:
- Date of purchase
- State where fuel was purchased (2-letter state code)
- Gallons purchased (number)
- Total amount paid (number, in dollars)
- Fuel type (diesel, gasoline, etc.)
- Station name/location (if visible)
...
If any information is not visible or unclear, use null for that field. Only return valid JSON, no additional text.`;
  ...
  try {
    ...
    if (mimeType.startsWith('image/')) {
      ...
      // For PDFs, we'd need to convert to image first or use a different approach
    ...
    // Fallback: return null values if JSON parsing fails
    return {
      date: null,
      state: null,
      gallons: null,
    ...
    console.error('Error parsing fuel receipt:', error);
    throw new Error(`Failed to parse fuel receipt: ${error.message}`);
  }
}
