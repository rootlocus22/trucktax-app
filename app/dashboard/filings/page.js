  ...
  const mobileFilingCards = allFilingsForTable.map((filing) => {
    ...
    return (
      <div key={filing.id || filing.draftId} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-4 relative overflow-hidden">
        ...
        <div className="pt-1">
          <Link href={resumeUrl} className="w-full text-xs font-bold bg-slate-50 hover:bg-slate-100 text-[var(--color-navy)] px-4 py-2.5 rounded-xl transition-colors flex items-center justify-center gap-1.5 border border-slate-200 shadow-sm">
            {isIncomplete ? 'Complete Draft' : 'View Status'} <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>
      </div>
    );
